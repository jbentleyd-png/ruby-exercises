...
def count_empty_seats(chart)
  ...
  p flat_chart
  flat_chart.reduce(0) do |empty_count, seat| 
    if seat == nil 
      empty_count += 1
      p empty_count
    else 
      empty_count #turns out we need this so it doesn't get dropped
    end
  end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.each do |hash_object|
    ...
    if hash_object[:is_my_favorite?] == true
      result = hash_object #the whole hash, not just the name lol
    end
  end
  p result
  result
